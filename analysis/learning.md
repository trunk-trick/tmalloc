...
It is used for memory allocation, I think it is its definition
But why are we here ? That is definitly to work out the theory of malloc !

So,let's begin:
And we will start with _int_malloc , to take a look at what's goin on when memory gets allocated. And we will definitly get something new

and we take aim at solving this problem--how it lowers the memory fragmentation rate :

we need to know abnout the basic structure of malloc's 

and we won't search the code one by one,it tends to be a memorizing, instead, we only look it up when necessary

it is an kind of art of bytes
...
firstly ,we use malloc in this way:
```cpp
#include <stdlib.h>
...
and the only input is the size we want to allocate, right ?
So ,when malloc get this size info,
it processes in this way:
```cpp
...
but why it is `2 * SIZE_SZ` ? As you can see that it is exactly the size of two headers `mchunk_size` and `mchunk_prev_size`. Thus it saves space while make the structure reason, or else we got to waste : like this below:
...
after considering the speed , easiness and cost, the order is :
![alt text](<Screenshot from 2026-07-15 13-31-54.png>) 

Nextly, we are going to work out the theory of fastbin :  
...
the annotation says : "While we're here, if we see other chunks of the same size, stash them in the tcache." Just like what we do above we get victim and redirect the `fb` pointer while the bin not empty and the tcache not full
...
and `fast_bin`'s range is limited,while it is fast, that is ,most of the case, we go to the regular bin.

and we also separate them base on their size : small_bin and large bin request
...
let's review the `bins` array I put above : 
...
That is : we got to know about how large chunks was arranged in large bins.
...
And if the size is not in the range of fastbin, it gose into the unsorted bin in regular bin.
Like what this branch dose: 
```C
